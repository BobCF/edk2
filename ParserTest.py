...
def TestDscParser(dsc_path,WorkspaceDir):
    ...
    for item in dsc_parser[MODEL_META_DATA_COMPONENT,"IA32"]:
        print(item)
    print("--------x64---------------")
    dsc_parser_x64 = DscParser(PathClass(dsc_path,WorkspaceDir),MODEL_FILE_DSC,"X64",
                                MetaFileStorage(PathClass(dsc_path,WorkspaceDir), MODEL_FILE_DSC))
    for item in dsc_parser_x64[MODEL_META_DATA_COMPONENT,"X64"]:
        print(item)
...
if __name__ == "__main__":
    WorkspaceDir = r"/home/coder/edk2"
